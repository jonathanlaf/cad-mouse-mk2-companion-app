...
fn list_hid_devices() -> Result<Vec<HidDeviceSummary>, String> {
  ...
  Ok(api.device_list().filter(|d| {
    d.usage_page() == 0xff60
      || d.product_string().is_some_and(|p| p.contains("CAD Mouse MK2"))
...
fn get_hid_feature(path: String) -> Result<Vec<u8>, String> {
  ...
  report[0] = 0x10;
  let len = device.get_feature_report(&mut report).map_err(|e| e.to_string())?;
  Ok(report[1..len.min(65)].to_vec())
}

#[tauri::command]
fn set_hid_feature(path: String, payload: Vec<u8>) -> Result<(), String> {
  if payload.len() != 64 { return Err("Feature payload must be exactly 64 bytes".into()); }
  let api = hidapi::HidApi::new().map_err(|e| e.to_string())?;
  let path = CString::new(path).map_err(|_| "Invalid HID path".to_owned())?;
  let device = api.open_path(path.as_c_str()).map_err(|e| e.to_string())?;
  let mut report = [0u8; 65];
  ...
  device.send_feature_report(&report).map_err(|e| e.to_string())?;
  Ok(())
}
...
pub fn run() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![list_hid_devices, get_hid_feature, set_hid_feature])
    .setup(|app| {
